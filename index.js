        ...
        for (var name in data) {
          if (name === 'Unfortunate 狗') {
            $.each(data[name].trans, function (j, num) {
              if (DEBUG)
                console.log(j, num);
              $(indexing[num]).addClass('right labeled icon').append('<i class="popup plane icon" data-html="<div class='header'>name</div><div class='content'><a target="_blank" href="' + data[name].site + num + '.html">連結</a></div>"></i>');
            });
          }
          else {
